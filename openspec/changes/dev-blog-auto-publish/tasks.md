...
> 実装状況の注記: 全タスク完了(2026-07-13)。初回記事(フェーズ0)が Zenn に公開され、
> 収集→生成→検査→PR→マージ→台帳追記→公開の全工程の動作を実運用で確認済み。
> セットアップの詳細は [docs/devblog-setup.md](../../../docs/devblog-setup.md) を参照してください。
...
- [x] 6.3 週次実行 1 回あたりの LLM コストを実測し、想定内であることを記録する — 初回成功実行で入力17,113/出力2,674トークン、約$0.036(5〜6円)/実行を実測し [tools/devblog/README.md](../../../tools/devblog/README.md) に記録済み(2026-07-12)
- [x] 6.4 運用手順（誤公開時のキーローテーション手順、許可リスト運用、再収集手順）を README に文書化する
- [x] 6.5 パイプライン全体をドライラン（実際の PR 作成まで）で通し、初回記事のレビュー・公開で動作を確認する — 2026-07-13、フェーズ0記事(PR #4)のマージ→台帳追記→Zenn 公開まで全て動作確認済み。公開URL: https://zenn.dev/techblog/articles/digest-2026-07-11t15-00-00-000z
