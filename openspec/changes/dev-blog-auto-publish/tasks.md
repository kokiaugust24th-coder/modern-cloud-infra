...
> 実装状況の注記: 2026-07-12 に初回のエンドツーエンド実行が成功し、PR #2 が公開用リポジトリに
> 作成されました。6.5 は PR のレビュー・マージ(人間の判断)のみ残っています。
...
- [ ] 6.5 パイプライン全体をドライラン（実際の PR 作成まで）で通し、初回記事のレビュー・公開で動作を確認する — PR 作成までは完走済み(2026-07-12、PR #2)。残るは人間による初回記事のレビュー・マージと、マージ時の台帳追記(record-publish.yml)の動作確認のみ
