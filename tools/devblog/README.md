...
**実測値(2026-07-12、初回成功実行: 35コミット分のダイジェスト、修復パスなしで一発合格)**

- 使用モデル: glm-5.2
- 合計トークン数: 入力 17,113 / 出力 2,674(outline 5,052+603 / draft 5,780+1,110 / critique 6,281+961)
- 概算コスト: 入力 $0.024 + 出力 $0.012 = **約 $0.036(約5〜6円)/ 1実行**
- リンター不合格時は外側リトライ(最大3回)+ 修復パス(最大2回/試行)でこの数倍になり得るが、
  それでも週次1回あたり数十円のオーダーに収まる
...
- `no-doubled-joshi`(助詞の重複)は無効化している(2026-07-12)。初回運用時、生成→修復パスを
  経てもこのルールだけが残り続けて4回の実行をブロックした。連体修飾を含む自然な日本語でも
  検出される誤検出に近い性質のルールであり、人間のPRレビューという最終ゲートが残っているため、
  スループットを優先して無効化した。文長・冗長表現・数値表記などの他ルールは維持している
